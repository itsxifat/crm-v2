...
export function amountToWords(amount, currency = 'BDT') {
  ...
  // Round the whole amount to cents first so .995+ carries into the whole part
  // (never "100 Paisa"), and work on the magnitude so negatives stay correct.
  ...
  let words = `${value < 0 && cents > 0 ? 'Minus ' : ''}${integerToWords(wholePart)} ${currencyLabel}`
  if (decimalPart > 0) {
