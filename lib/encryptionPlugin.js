  ...
  function encryptDoc(doc) {
    ...
    for (const { path } of fields) {
      ...
      if (value !== null && value !== undefined) {
        set(doc, path, encrypt(value, collection, path))
        // Required for Mixed-type fields (pricing, hourlyRate) so Mongoose
        // includes the changed value in the save operation.
    ...
    for (const { path, indexField } of blindIndexes) {
      ...
      if (original !== null && original !== undefined) {
        doc[indexField] = makeBlindIndex(original, collection, path)
      }
    }
  }
