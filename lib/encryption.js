...
export function decrypt(value, collection, field, originalType = 'string') {
  ...
  try {
    ...
    switch (originalType) {
      ...
      case 'date': {
        // Dates were JSON-serialised before encryption, so the plaintext is
        // usually a quoted ISO string ('"1990-05-01T…"'). Unquote it first.
        let s = plain
        if (s.startsWith('"')) { try { s = JSON.parse(s) } catch { /* keep raw */ } }
        const d = new Date(s)
