...
// `to` is the WhatsApp number; `email` is the login email shown in the message.
export async function sendEmployeeLoginWhatsApp({ to, email, name, password }) {
  if (!to) return
  ...
  try {
    await sendTemplate({
      ...
      params:       [name || 'there', email || 'your registered email', password, `${appUrl}/login`],
      purpose:      'general',
    })
  } catch (err) {
...
// Template: enfinito_profile_approved (reused) — freelancer/agency KYC approved.
export async function sendFreelancerApprovedWhatsApp({ to, name }) {
  if (!to) return
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  ...
  try {
    await sendTemplate({
      to,
      templateName: 'enfinito_profile_approved',
      params:       [name || 'there', `${appUrl}/freelancer`],
      purpose:      'general',
    })
  } catch (err) {
    console.error('[WhatsApp] sendFreelancerApprovedWhatsApp failed:', err.message)
  }
}

// Template: enfinito_freelancer_invite
// Body: Hi {{1}} 👋 — added as {{2}}, Email: {{3}}, Password: {{4}}, Login: {{5}}, Setup: {{6}}
export async function sendFreelancerInviteWhatsApp({ to, email, name, link, type = 'FREELANCER', password }) {
  if (!to) return
  const appUrl    = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  ...
  try {
    await sendTemplate({
      ...
      params:       [name || 'there', typeLabel, email || 'your registered email', password, `${appUrl}/login`, link],
      purpose:      'freelancer',
    })
  } catch (err) {
...
export async function sendClientWelcomeWhatsApp({ to, email, name, clientCode, password }) {
  if (!to) return
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  ...
  try {
    await sendTemplate({
      ...
      params:       [name, clientCode, email || 'your registered email', password, `${appUrl}/login`],
      purpose:      'client',
    })
  } catch (err) {
