  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      if (user) {
        ...
        // CLIENT with exactly one company → pre-select it (no chooser needed)
        if (user.role === 'CLIENT') {
          await connectDB()
        ...
        if (!token.roleSyncedAt || Date.now() - token.roleSyncedAt > SYNC_INTERVAL) {
          ...
          if (freshUser) {
            ...
            // Self-heal: a CLIENT with no active company but exactly one
            // membership gets it auto-selected (covers sessions issued before
            // this field existed, or a user added to their first company).
            if (freshUser.role === 'CLIENT' && !token.activeClientId) {
