...
export const authOptions = {
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        // IP is for the audit log only — derived from the trusted proxy hop, never
        // the client-supplied first X-Forwarded-For entry.
        const ip  = clientIpFromHeaders(h => req?.headers?.[h], req?.socket?.remoteAddress || 'unknown')
        const id  = credentials.identifier.trim()
        // Lock out per identifier (not per IP) so rotating X-Forwarded-For values
        // cannot reset the failure counter.
        const rlKey = `id:${id.toLowerCase()}`
        ...
        // Client IDs (ENCL-…) are matched before phone so a phone value can
        // never shadow another user's Client-ID login.
        const looksLikeClientCode = /^ENCL-/i.test(id)
        if (!user && looksLikeClientCode) {
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      // Revocation check on EVERY evaluation of an existing session (the initial
      // sign-in is already vetted by authorize()). A deleted or deactivated user
      // loses access immediately: throwing here makes next-auth v4's session
      // handler (core/routes/session.js) skip the session callback and return an
      // empty body, so getServerSession() → null (every `if (!session)` guard
      // blocks), useSession() → unauthenticated, and /api/auth/session clears
      // the session cookie. Returning null instead would NOT work in v4 — the
      // session callback would still run and yield a non-empty session.
      if (!user) {
        if (!token?.id) throw new Error('SESSION_REVOKED: token has no user id')
        await connectDB()
        const live = await User.findById(token.id).select('isActive').lean()
        if (!live || !live.isActive) {
          throw new Error('SESSION_REVOKED: account deactivated or deleted')
        }
      }
