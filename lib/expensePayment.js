...
// and flips any linked FreelancerAssignment / SalaryPayout. Payment details
// (method / txn / proof) are already recorded at approval; here we just attach
// the authorized-invoice scan and settle. The caller owns the APPROVED-status +
// authorized-scan guards. Returns the created Transaction.
export async function settleExpenseAsPaid(expense, {
  userId, signedInvoiceUrl = null, accountManager, batchInvoiceNo = null,
}) {
  if (signedInvoiceUrl) expense.signedInvoiceUrl = signedInvoiceUrl
  ...
  const txn = await new Transaction({
    ...
    receiptUrl:      expense.signedInvoiceUrl ?? expense.paymentProofUrl ?? expense.invoiceUrl ?? null,
