 ...
 *
 * New flat-string RBAC (preferred for new code):
 *   canDo(session, 'sales.leads.create')
 *   requirePerm(session, 'projects.delete')  // returns 403 NextResponse or null
 */

// Re-export new flat RBAC helpers so callers only need one import
export { canDo, canDoAny, requirePerm, hasPermission, hasAnyPermission } from '@/lib/rbac'
...
// Legacy resource:action pairs used by canAccess() call sites whose verb does
// not match the flat permission vocabulary (lib/rbac.js).
const LEGACY_FLAT_MAP = {
  'invoices:read':           ['sales.invoices.view'],
  'accounts:view':           ['finance.accounts.view'],
  'accounts:addTransaction': ['finance.transactions.create'],
}
...
export function sessionCan(sessionUser, resource, action) {
  ...
  // New flat permissions array — check if any permission in this array
  // starts with the resource and contains the action. The array is
  // authoritative even when empty (matches rbac.canDo) — no default fallback.
  if (Array.isArray(permissions)) {
    // Legacy verbs that have no same-named flat permission map explicitly.
    const mapped = LEGACY_FLAT_MAP[`${resource}:${action}`]
    if (mapped) return mapped.some(p => permissions.includes(p))
    // Build possible flat keys for this resource+action combo
    // e.g. resource='leads' action='view' → 'sales.leads.view'
    const match = permissions.some(p => {
      const parts = p.split('.')
      // parts could be ['sales','leads','view'] or ['projects','view'] etc.
      const lastPart = parts[parts.length - 1]
      const resourcePart = parts[parts.length - 2] ?? parts[0]
      return (resourcePart === resource || parts.includes(resource)) && lastPart === action
    })
    return match
  }

  // Legacy nested object format (kept for backward compatibility)
