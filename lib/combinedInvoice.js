 ...
 */
export function projectInvoiceFilter(projectIdOrDoc) {
  const projectId = projectIdOrDoc?._id ?? projectIdOrDoc
  // A missing/invalid id (e.g. a combined invoice whose project was deleted) must
  // match NOTHING — `{ projectId: null }` would match every project-less invoice
  // across all clients.
...
export function deriveStatus(children = [], totals) {
  ...
  // A due date is a calendar day (stored 00:00Z): overdue only from the start
  // of the next Asia/Dhaka day.
  const todayStart = dhakaDayStart()
  const anyOverdue = children.some(c =>
    c.status === 'OVERDUE' ||
    (c.dueDate && new Date(c.dueDate) < todayStart && c.due > 0.01)
  )
 ...
 */
export async function findProjectInvoices(projectId, { billableOnly = true, populate = true, clientId = null } = {}) {
  const filter = projectInvoiceFilter(projectId)
  if (billableOnly) filter.status = { $nin: NON_BILLABLE_STATUSES }
  if (clientId) filter.clientId = toObjectId(clientId._id ?? clientId)
...
export async function buildCombined(combinedDoc, { includePayments = true } = {}) {
  ...
  // After populate, a deleted project comes back as null — never fall back to a
  // null id (see projectInvoiceFilter). Children are also scoped to the combined
  // invoice's own client.
  const populatedId = typeof combinedDoc.populated === 'function' ? combinedDoc.populated('projectId') : undefined
  const projectId = populatedId ?? combinedDoc.projectId?._id ?? combinedDoc.projectId ?? null
  const clientId  = combinedDoc.clientId?._id ?? combinedDoc.clientId ?? null
  const invoices  = projectId ? await findProjectInvoices(projectId, { clientId }) : []
  ...
  const excludedCount = projectId
    ? await Invoice.countDocuments({
      ...projectInvoiceFilter(projectId),
      ...(clientId ? { clientId: toObjectId(clientId) } : {}),
  ...
  if (includePayments && children.length > 0) {
    payments = await ProjectPayment.find({
      ...
      .lean()
    // Expose the ledger reference as `txnId` for the print view's Reference column
    payments = payments.map(p => ({ ...p, id: p._id.toString(), txnId: p.transactionId?.txnId ?? null }))
  }
...
export async function ensureCombinedInvoice(projectId, { createdBy = null, force = false } = {}) {
  ...
  try {
    ...
    if (err?.code === 11000) {
      ...
      const theirs = await CombinedInvoice.findOne({ projectId: toObjectId(projectId) })
      if (theirs) return theirs
      // Otherwise the collision was on combinedNumber — retry once with a fresh number.
      if (err?.keyPattern?.combinedNumber || /combinedNumber/.test(String(err?.message))) {
