...
export const PERMISSION_MODULES = [
  ...
  },
  {
    ...
    permissions: [
      ...
      { key: 'sales.customers.passwordReset', label: 'Approve Client Password Resets' },
  ...
  },
  {
    ...
    permissions: [
      ...
      { key: 'finance.expenses.submit',     label: 'Submit Expense / Reimbursement' },
      { key: 'finance.salary.view',         label: 'View Salary' },
      { key: 'finance.salary.pay',          label: 'Generate & Pay Salary' },
  ...
  {
    // Field-level visibility of personal / confidential data. When a role lacks
    // one of these, the matching fields are partially masked (e.g. j•••@gmail.com)
    // in API responses. SUPER_ADMIN holds all of these by default; grant
    // individually to trust a role with specific categories of PII.
    key: 'pii',
    label: 'Confidential Data',
    permissions: [
      { key: 'pii.contact.view',  label: 'View Email & Phone' },
      { key: 'pii.address.view',  label: 'View Addresses' },
      { key: 'pii.identity.view', label: 'View Identity / KYC Numbers' },
      { key: 'pii.financial.view',label: 'View Salaries, Rates & Bank Details' },
      { key: 'pii.export',        label: 'Export Unmasked Data' },
    ],
  },
  {
    key: 'account',
...
export const DEFAULT_ROLE_PERMISSIONS = {
  ...
  MANAGER: [
    ...
    'sales.quotations.view', 'sales.quotations.create', 'sales.quotations.update', 'sales.quotations.approve',
    ...
    'finance.payments.request', 'finance.payments.confirm', 'finance.expenses.submit',
    'finance.salary.view', 'finance.salary.pay',
  ...
  EMPLOYEE: [
    'dashboard.view',
    // Sales — leads (no delete); quotations create/edit; read-only customers & invoices
    ...
    // Finance — payment request + submit own expenses/reimbursements
    'finance.payments.request', 'finance.expenses.submit',
...
/**
 * Apply a person's individual permission overrides on top of a baseline set.
 * Baseline is the assigned custom role's permissions (or the base-role defaults
 * when no custom role is set). `added` grants extra permissions to just this
 * person; `removed` revokes ones the role would otherwise give them.
 *
 *   effective = (base ∪ added) − removed
 *
 * @param {string[]|null|undefined} base — baseline permission strings
 * @param {{ added?: string[], removed?: string[] }|null|undefined} overrides
 * @returns {string[]}
 */
export function applyPermissionOverrides(base, overrides) {
...
/**
 * Does a person have any individual overrides worth applying?
 * @param {{ added?: string[], removed?: string[] }|null|undefined} overrides
 * @returns {boolean}
 */
export function hasPermissionOverrides(overrides) {
  return !!(overrides?.added?.length || overrides?.removed?.length)
}
 ...
 * - EMPLOYEE / MANAGER with a permissions ARRAY (custom role and/or individual
 *   overrides) → that array is authoritative, even when empty (empty = no access).
 * - EMPLOYEE / MANAGER with permissions null/undefined (no custom role, no
 *   overrides) → fall back to DEFAULT_ROLE_PERMISSIONS.
 *
...
export function canDo(session, permission) {
  ...
  // A resolved permissions array is authoritative — an EMPTY array means the
  // custom role / overrides grant nothing, so it must NOT fall back to defaults.
  if (Array.isArray(permissions)) {
    return permissions.includes(permission)
  }

  // Only null/undefined (no custom role, no overrides) uses the role defaults.
...
// Internal (staff) roles — everything else (CLIENT/FREELANCER/VENDOR) is external.
const STAFF_ROLES = ['SUPER_ADMIN', 'MANAGER', 'EMPLOYEE']

/**
 * Server-side guard for admin/staff-only endpoints. Middleware does NOT enforce
 * roles on /api routes, so admin data endpoints must guard themselves or any
 * authenticated user (including clients/freelancers/vendors) can read them.
 *
 * Returns a 401/403 NextResponse to short-circuit, or null to proceed.
 *
 * Usage:
 *   const denied = requireStaff(session); if (denied) return denied
 *
 * @param {object} session — next-auth session object
 * @returns {NextResponse|null}
 */
...
/**
 * Stricter guard for HR / vendor / finance-sensitive endpoints — SUPER_ADMIN or
 * MANAGER only (excludes EMPLOYEE). Returns a 401/403 NextResponse or null.
 */
...
// Single source of truth for which permission is required to *open* an admin
// page. Consumed by the client-side <RouteGuard> (blocks direct-URL access to a
// page the user's role/permissions don't allow) so the frontend enforcement
// matches the sidebar visibility and the API-route guards.
//
// Matched by longest path prefix, so more specific routes must come first.
// A route with `null` permission is always reachable for staff (e.g. own profile).
export const ROUTE_PERMISSIONS = [
  // Sales
  { path: '/admin/leads',             perm: 'sales.leads.view' },
  { path: '/admin/clients',           perm: 'sales.customers.view' },
  { path: '/admin/password-requests', perm: 'sales.customers.passwordReset' },
  { path: '/admin/quotations',        perm: 'sales.quotations.view' },
  { path: '/admin/invoices',          perm: 'sales.invoices.view' },
  // Projects & tasks
  { path: '/admin/projects',          perm: 'projects.view' },
  { path: '/admin/tasks',             perm: 'tasks.view' },
  // Finance — the accounts page hosts several tabs, each reached from its own
  // permissioned sidebar link and self-gated in-page. Entry is allowed if the
  // user holds ANY finance tab permission (array = any-of).
  { path: '/admin/accounts',          perm: [
    'finance.overview.view', 'finance.transactions.view',
    'finance.payments.confirm', 'finance.payments.request',
    'finance.withdrawals.view', 'finance.reports.view',
  ] },
  { path: '/admin/my-expenses',       perm: 'finance.expenses.submit' },
  { path: '/admin/salary',            perm: 'finance.salary.view' },
  // HR & People
  { path: '/admin/employees',         perm: 'hr.employees.view' },
  { path: '/admin/freelancers',       perm: 'hr.freelancers.manage' },
  { path: '/admin/vendors',           perm: 'hr.vendors.manage' },
  { path: '/admin/agencies',          perm: 'hr.agencies.manage' },
  { path: '/admin/attendance',        perm: 'hr.attendance.view' },
  { path: '/admin/leaves',            perm: 'hr.leaves.view' },
  { path: '/admin/roles',             perm: 'hr.roles.manage' },
  // Analytics
  { path: '/admin/reports',           perm: 'analytics.reports.view' },
  // System
  { path: '/admin/config',            perm: 'system.config.view' },
  { path: '/admin/settings',          perm: 'system.config.view' },
  { path: '/admin/activity-logs',     perm: 'system.logs.view' },
  // Account — always reachable so anyone can complete their own profile/KYC.
  { path: '/admin/profile',           perm: null },
  { path: '/admin/notifications',     perm: 'account.notifications.view' },
  { path: '/admin/account',           perm: 'account.profile.view' },
]

/**
 * Resolve the permission required to open a given admin pathname.
 * Returns the permission string, or null when the route is unmapped or
 * intentionally open (e.g. the dashboard root and own-profile pages).
 * @param {string} pathname
 * @returns {string|null}
 */
export function getRoutePermission(pathname) {
  if (!pathname) return null
  // Longest-prefix wins so /admin/leads/123 resolves the same as /admin/leads.
