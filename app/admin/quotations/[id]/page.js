...
function ItemCard({ item, idx, onChange, onRemove, editing, ventures, ventureCategories }) {
  ...
  const ventureLabel = ventures.find(v => v.id === item.venture)?.label ?? item.venture
...
function QuotationPrintView({ quotation: q, company = {}, ventures = [] }) {
        ...
        <tbody>
          {(q.items ?? []).map((item, i) => (
            <tr key={i}>
              ...
              <td style={TD({})}>
                {item.venture && <p style={{ margin: '0 0 2px', fontSize: 10, fontWeight: 500, color: '#94a3b8', lineHeight: 1.3, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{ventures.find(v => v.id === item.venture)?.label ?? item.venture}</p>}
  ...
  if (printMode) {
    return (
      <div>
        ...
        <QuotationPrintView quotation={q} company={company} ventures={ventures} />
      </div>
    )
  }
